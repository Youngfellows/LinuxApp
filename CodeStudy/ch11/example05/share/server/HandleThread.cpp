...
void HandleThread::start()
{
    ...
    // cout << "HandleThread::start():: 启动一个线程执行,thread_id:" << processThread->get_id() << endl;
}
...
{
    cout << "HandleThread::process():: start ..." << endl;
    ...
    this->manager->receiveRemote(); // //接收服务端数据
    //this->manager->sendRemote();    //调用发送数据
    cout << "HandleThread::process():: end ..." << endl;
}
 ...
 *
 */
void HandleThread::process2()
{
    for (int i = 0; i < 5; i++)
    {
        cout << "HandleThread::process2():: " << i << " ,start ..." << endl;
        std::thread::id this_id = std::this_thread::get_id();
        unsigned int t = *(unsigned int *)&this_id; // threadid 转成 unsigned int
        unsigned int threadid = t;
        int id = syscall(SYS_gettid);
        g_display_mutex.lock();
        std::cout << "HandleThread::process2():: this_id:" << this_id << " ,sleeping...\n";
        std::cout << "HandleThread::process2():: threadid:" << threadid << endl;
        std::cout << "HandleThread::process2():: id:" << id << endl;
        g_display_mutex.unlock();
        std::this_thread::sleep_for(std::chrono::seconds(1));
        // this->manager->sendRemote(); //调用发送数据
        cout << "HandleThread::process2():: " << i << " ,end ..." << endl;
    }
}
