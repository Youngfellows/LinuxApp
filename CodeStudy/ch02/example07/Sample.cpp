...
void test1(int arg, char *args[])
{
    ...
    /////////////////////////////////////////////////////////////////////////
    char *para_l = nullptr; //参数c
    int optchar;            //获取命令行参数
    opterr = 0;             //返回？代表出错

    while ((optchar = getopt(arg, args, "l:")) != -1)
    {
        cout << "test1():: optchar:" << optchar << endl;
        switch (optchar)
        {
        case 'l':
            para_l = optarg; //冒号:或者双冒号::后面跟随的参数
            checkAndDisplay(para_l);
            break;
        case '?': //出错
            if (optopt == 'l')
            {
